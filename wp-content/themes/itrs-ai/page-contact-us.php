    ...
    <div class="grid gap-8 md:grid-cols-[0.9fr_1.1fr] md:items-start">
        ...
        <article class="rounded-4xl border border-brand/20 bg-white p-8 shadow-[0_20px_55px_-36px_rgba(243,130,80,0.55)]">
            <?php if ('success' === $form_status) : ?>
                <p class="mb-6 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">Thank you. Your message has been sent successfully.</p>
            <?php elseif ('spam_blocked' === $form_status || 'rate_limited' === $form_status) : ?>
                <p class="mb-6 rounded-xl border border-rose-300 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">Too many attempts detected. Please wait a few minutes and try again.</p>
            ...
            <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" class="grid gap-4">
                ...
                <label class="grid gap-1 text-sm font-medium text-slate-700">Message
                    <textarea name="message" required rows="6" class="rounded-xl border border-brand/25 bg-amber-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-brand"></textarea>
                </label>

                <button type="submit" class="mt-2 inline-flex w-fit rounded-xl bg-brand px-6 py-3 text-sm font-semibold text-white transition hover:brightness-110">Send Message</button>
            </form>
        </article>
        <?php endwhile; endif; ?>
    </div>
